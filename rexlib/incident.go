...
type Provider interface {
	InitSchema(handle *IncidentHandle) []tsfile.TSFPageTag
	
	Gather(handle *IncidentHandle)
}
...
func (incident *Incident) run(handle *IncidentHandle) {
	...
	for {
		...
		for provIndex, _ := range incident.Providers {
			...
			if prov.StartedAt.IsZero() {
				...
				handle.Providers[provIndex].InitSchema(handle)
			}
			if prov.StoppedAt.IsZero() {
				handle.Providers[provIndex].Gather(handle)
				provCount++	
			}
		}
